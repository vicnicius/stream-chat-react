...
import { Chat as DefaultChat, CustomChat } from '..';
import {
  ChatContext,
  EmojiContext,
  TranslationContext,
...
describe('Chat components', () => {
  describe.each([
    ...
    describe('active channel', () => {
      ...
      it('setActiveChannel prevent event default', async () => {
        ...
        await waitFor(() =>
          expect(context.setActiveChannel).not.toBeUndefined(),
        );
  ...
  describe('CustomChat EmojiContext', () => {
    afterEach(cleanup);
    ...
    it('should expose default emoji components if none is assigned', async () => {
      let context;
      const chatClient = getTestClient();
      const emojiData = {
        compressed: true,
      ...
      render(
        <CustomChat client={chatClient} emojiData={emojiData}>
          <EmojiContextConsumer
            fn={(ctx) => {
              context = ctx;
            }}
      ...
      await waitFor(() => {
        expect(context).toBeInstanceOf(Object);
        expect(context.emojiData).toBe(emojiData);
        expect(context.EmojiPicker).toBe(DefaultEmojiPicker);
        expect(context.Emoji).toBe(DefaultEmoji);
      });
    });
  });
});
